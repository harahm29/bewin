			...
			<div class="content">
				<div class="page-inner">
					<div class="page-header">
						<h4 class="page-title">Agent Transaction</h4>
						<ul class="breadcrumbs">
							...
							<li class="nav-item">
								<a href="{{url()->current()}}">Agent Transaction</a>
							</li>
							<li class="separator">
								<i class="flaticon-right-arrow"></i>
							</li>
			...
			<table id="firsttab" class="table table-bordered table-striped">
              ...
              <tr>
				...
				<th colspan="2" style="text-align: right;">
				   ...
				   @if(Auth::user()->type=='admin')
					   @if(!empty($from))
					<a target="_blank" href="{{url('agent-transaction-pdf/'.$search.'/'.$from.'/'.$to)}}" class="btn btn-primary view_pdf"><i class="far fa-file-pdf"></i> View Pdf</a>
				      @endif
					  @else
						  @php  $search = Auth::user()->id; @endphp
						  <a target="_blank" href="{{url('agent-transaction-pdf/'.$search.'/'.date('Y-m-d',strtotime($from)).'/'.date('Y-m-d',strtotime($to)))}}" class="btn btn-primary view_pdf btn-sm"><i class="far fa-file-pdf"></i> View Pdf</a>
					  @endif
					</th>
